  ...
  const addTask = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_BACKEND_LINK}/api/v1/addTask`,
        Values,
        {
  ...
  return (
    <div className="bg-white rounded px-4 py-4 w-[70%] md:w-[40%]">
      ...
      <form method="POST" className="flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between gap-4">
          <button
            className="w-full rounded-md bg-blue-500 py-2 text-white hover:bg-blue-800 transition-all duration-300 text-white-rounded"
            onClick={addTask}
          >
            Add Task{" "}
          </button>
          <button
            className="w-full rounded-md border border-black py-2 hover:bg-zinc-200 transition-all duration-300 text-white-rounded"
            onClick={(e) => {
              e.preventDefault();
              setAddTaskDiv("hidden");
            }}
