    ...
    const fetch = async () => {
      try {
        ...
        const res = await axios.get(
          `${import.meta.env.VITE_BACKEND_LINK}/api/v1/getTask/${EditTaskId}`,
          {
  ...
  const editTask = async (e, id) => {
    ...
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_BACKEND_LINK}/api/v1/editTask/${id}`,
        Values,
        {
          withCredentials: true,
        }
  ...
  const deleteTask = async (e, id) => {
    ...
    try {
      const res = await axios.delete(
        `${import.meta.env.VITE_BACKEND_LINK}/api/v1/deleteTask/${id}`,
        {
          withCredentials: true,
        }
      );
    ...
    <div className="bg-white rounded px-4 py-4 w-[70%] md:w-[40%]">
      ...
      <form method="POST" className="flex flex-col gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <button
            className="w-full sm:w-auto rounded-md bg-blue-500 py-2 px-4 text-white hover:bg-blue-800 transition-all duration-300 text-white-rounded"
            onClick={(e) => editTask(e, Values._id)}
          >
            Edit Task{" "}
          </button>
          <button
            className="sm:w-auto w-full px-4 rounded-md border border-red-500  text-red-600 py-2 hover:bg-red-200 transition-all duration-300 text-white-rounded"
          ...
          <button
            className="sm:w-auto w-full rounded-md border border-black py-2 px-4 hover:bg-zinc-200 transition-all duration-300 text-white-rounded"
            onClick={(event) => {
