...
const Dashboard = () => {
    ...
    const fetchUserDetails = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BACKEND_LINK}/api/v1/userDetails`,
          {
  ...
  return (
    <div className="w-full relative ">
      ...
      <div
        className={`w-full ${AddTaskDiv}  h-screen fixed top-0 left-0 bg-zinc-800 opacity-85 flex items-center justify-center z-50`}
      ></div>
      <div
        className={`w-full ${AddTaskDiv} h-screen fixed top-0 left-0 flex items-center justify-center   z-50`}
      >
        <AddTask setAddTaskDiv={setAddTaskDiv} />
      </div>

      <div
        className={`w-full ${EditTaskDiv}  h-screen fixed top-0 left-0 bg-zinc-800 opacity-85 flex items-center justify-center z-50`}
      ></div>
      <div
        className={`w-full ${EditTaskDiv} h-screen fixed top-0 left-0 flex items-center justify-center z-50`}
      >
